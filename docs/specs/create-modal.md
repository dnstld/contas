...
The "Adicionar" modal is the primary mutation entry point of the app. It is invoked from the Home header's "+ Adicionar" button and presented above the Home stack.

The route itself lives at `app/(modals)/modal.tsx` (URL: `/modal`) and is currently a placeholder screen. This spec pins down the trigger, presentation, and dismissal contracts so the future form implementation can extend it without breaking those guarantees.
...
When the user navigates to the route /modal (programmatically or via the "Adicionar" button)
...
```
Given that the user taps the Home header "Adicionar" button
When the modal is presented
Then it must use the SlideFromBottom transition declared in app/_layout.tsx
And the Home screen must remain mounted in the stack underneath the modal
And the modal must not replace the Home screen (it is presented on top, not navigated to)
And the modal must visually occupy the screen above the underlying Home content
```

### Dismissal — back to Home
...
And the user must return to the Home screen at the same scroll position and filter state as before
...
When no explicit /modal link is in the deep-link path
...
And tapping the link must call `router.dismissTo` and return to the Home screen
